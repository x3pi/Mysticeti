...
tmux new-session -d -s "${GO_MASTER_SESSION[$NODE_ID]}" -c "$GO_SIMPLE_ROOT" \
    "export GOTOOLCHAIN=go1.23.5 && export XAPIAN_BASE_PATH='$XAPIAN_MASTER' && go run . -config=${GO_MASTER_CONFIG[$NODE_ID]} >> \"$LOG_DIR/node_$NODE_ID/go-master-stdout.log\" 2>&1"
...
tmux new-session -d -s "${GO_SUB_SESSION[$NODE_ID]}" -c "$GO_SIMPLE_ROOT" \
    "export GOTOOLCHAIN=go1.23.5 && export XAPIAN_BASE_PATH='$XAPIAN_SUB' && go run . -config=${GO_SUB_CONFIG[$NODE_ID]} >> \"$LOG_DIR/node_$NODE_ID/go-sub-stdout.log\" 2>&1"
