...
# - Node 0: Validator (có executor - node đặc biệt)
...
print_info "🚀 Khởi động Node $NODE_ID (Validator with Executor) trong tmux session '$TMUX_SESSION'..."

# Start in tmux
print_info "Creating tmux session '$TMUX_SESSION'..."
if ! tmux new-session -d -s "$TMUX_SESSION" -c "$METANODE_ROOT" \
    "$BINARY start --config $CONFIG_FILE 2>&1 | tee $METANODE_ROOT/logs/latest/node_0.log" 2>/dev/null; then
...
if [ -S "/tmp/executor$NODE_ID.sock" ]; then
    print_info "  ✅ Executor socket: /tmp/executor$NODE_ID.sock"
else
