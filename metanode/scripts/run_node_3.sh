...
# Script khởi động Rust Consensus Node 3 riêng biệt
# - Node 3: Validator (không có executor)
# - Sử dụng config/node_3.toml
# - Chạy trong tmux session 'metanode-3'
...
if ! tmux new-session -d -s "$TMUX_SESSION" -c "$METANODE_ROOT" \
    "$BINARY start --config $CONFIG_FILE 2>&1 | tee $METANODE_ROOT/logs/latest/node_3.log" 2>/dev/null; then
