...
# Script khởi động Rust Consensus Node 2 riêng biệt
# - Node 2: Validator (không có executor)
# - Sử dụng config/node_2.toml
# - Chạy trong tmux session 'metanode-2'
...
print_info "🚀 Khởi động Node $NODE_ID (Validator) trong tmux session '$TMUX_SESSION'..."

# Start in tmux
print_info "Creating tmux session '$TMUX_SESSION'..."
if ! tmux new-session -d -s "$TMUX_SESSION" -c "$METANODE_ROOT" \
    "$BINARY start --config $CONFIG_FILE 2>&1 | tee $METANODE_ROOT/logs/latest/node_2.log" 2>/dev/null; then
